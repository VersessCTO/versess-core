...
#include "ui_zverscontroldialog.h"
...
std::set<std::string> ZVersControlDialog::setSelectedMints;
std::set<CMintMeta> ZVersControlDialog::setMints;

bool CZVersControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZVersControlDialog::COLUMN_DENOMINATION || column == ZVersControlDialog::COLUMN_VERSION || column == ZVersControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZVersControlDialog::ZVersControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZVersControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZVersControlDialog::~ZVersControlDialog()
{
    delete ui;
}

void ZVersControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZVersControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZVersControlWidgetItem* itemDenom(new CZVersControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZVersControlWidgetItem *itemMint = new CZVersControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zversTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zversTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zVERS.";
            else if (!mint.isSeedCorrect)
                strReason = "The zVERS seed used to mint this zVERS is not the same as currently hold in the wallet";
            else
...
void ZVersControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZVersControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZVers_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZVersControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZVersControlDialog::GetSelectedMints()
{
...
void ZVersControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
