...
public:
    ...
    bool WriteZVERSSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZVERSSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZVERSSeed_deprecated(uint256& seed);
    bool EraseZVERSSeed();
    bool EraseZVERSSeed_deprecated();

    bool WriteZVERSCount(const uint32_t& nCount);
    bool ReadZVERSCount(uint32_t& nCount);
