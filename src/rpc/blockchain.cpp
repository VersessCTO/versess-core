...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    UniValue zversObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zversObj.push_back(Pair(to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zversObj.push_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
    result.push_back(Pair("zVERSsupply", zversObj));
...
UniValue getchecksumblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "  \"zVERSsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zVERS denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zVERS denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zVERS denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zVERS denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zVERS denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zVERS denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zVERS denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zVERS denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zVERS denominations\n"
            "  }\n"
            "}\n"
...
UniValue getrawmempool(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in versess\n"
...
UniValue gettxout(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of versess addresses\n"
            "     \"versessaddress\"   	 	(string) versess address\n"
            "        ,...\n"
            "     ]\n"
            "  },\n"
...
UniValue getaccumulatorwitness(const UniValue& params, bool fHelp)
{
    ...
    if (!GenerateAccumulatorWitness(pubCoin, accumulator, witness, nMintsAdded, strFailReason)) {
        receipt.SetStatus(_(strFailReason.c_str()), ZVERS_FAILED_ACCUMULATOR_INITIALIZATION);
