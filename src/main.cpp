...
#include "zvers/accumulatormap.h"
#include "addrman.h"
#include "alert.h"
...
#if defined(NDEBUG)
#error "Versess cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uvers) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend* spend, CBlockIndex* pindex, const uint256& hashBlock)
{
	...
	if (IsSerialInBlockchain(spend->getCoinSerialNumber(), nHeightTx))
		return error("%s : zVERS spend with serial %s is already in block %d\n", __func__,
...
{
	//Check to see if the zVERS is properly signed
	if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
		try {
			if (!spend->HasValidSignature())
				return error("%s: V2 zVERS spend does not have a valid signature\n", __func__);
		}
		...
		if (spend->getSpendType() != expectedType) {
			return error("%s: trying to spend zVERS without the correct spend type. txid=%s\n", __func__,
				tx.GetHash().GetHex());
		}
	}
	...
	//    if (v1Serial) {
	//        return error("%s : zVERS v1 serial spend not spendable, serial %s, tx %s\n", __func__,
	...
	//        return error("%s : zVERS spend with serial %s from tx %s is not in valid range\n", __func__,
	//                 spend->getCoinSerialNumber().GetHex(), tx.GetHash().GetHex());
	//    else
...
bool CheckZerocoinSpend(const CTransaction& tx, bool fVerifySignature, CValidationState& state)
{
	...
	for (const CTxIn& txin : tx.vin) {
		...
		if (isPublicSpend) {
			...
			if (!ZVERSModule::parseCoinSpend(txin, tx, prevOut, publicSpend)) {
		...
		if (isPublicSpend) {
			...
			if (!ZVERSModule::validateInput(txin, prevOut, tx, ret)) {
...

	{
		...
		if (tx.HasZerocoinSpendInputs()) {
			...
			if (IsTransactionInChain(tx.GetHash(), nHeightTx))
				return state.Invalid(error("AcceptToMemoryPool : zVERS spend tx %s already in block %d",
			...
			for (const CTxIn& txIn : tx.vin) {
				...
				if (!isPrivZerocoinSpend && !isPublicSpend) {
					return state.Invalid(error("%s: AcceptToMemoryPool failed for tx %s, every input must be a zcspend or zcpublicspend", __func__,
						tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zvers");
				}
				...
				if (isPublicSpend) {
					libzerocoin::ZerocoinParams* params = Params().Zerocoin_Params();
					PublicCoinSpend publicSpend(params);
					if (!ZVERSModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)) {
						return false;
					}
			...
			// Check that zVERS mints (if included) are not already known
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight <= Params().LAST_POW_BLOCK() && nHeight >= 1) {
		...
		nSubsidy = 0.5 * COIN;
	} else if (nHeight <= 70000 && nHeight > 20000) {
		nSubsidy = 0.6 * COIN;
	} else if (nHeight <= 100000 && nHeight > 70000) {
		nSubsidy = 0.7 * COIN;
	} else if (nHeight <= 160000 && nHeight > 100000) {
		nSubsidy = 0.58 * COIN;
		...
		nSubsidy = 0.2 * COIN;
	} else if (nHeight <= 296955000 && nHeight > 700000) {
		nSubsidy = 0.1 * COIN;
	} else {
		nSubsidy = 0 * COIN;
	}
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZVERSStake)
{
	int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
	...
	for (int i = block.vtx.size() - 1; i >= 0; i--) {
		...
		* note we only undo zerocoin databasing in the following statement, value to and from VERSESS
...
{
	RenameThread("versess-scriptch");
	scriptcheckqueue.Thread();
}

void RecalculateZVERSMinted()
{
	CBlockIndex *pindex = chainActive.Genesis();
	uiInterface.ShowProgress(_("Recalculating minted ZVERS..."), 0);
	while (true) {
		...
		if (pindex->nHeight % 1000 == 0) {
			...
			uiInterface.ShowProgress(_("Recalculating minted ZVERS..."), percent);
		}
...
void RecalculateZVERSSpent()
{
	CBlockIndex* pindex = chainActive.Genesis();
	uiInterface.ShowProgress(_("Recalculating spent ZVERS..."), 0);
	while (true) {
		if (pindex->nHeight % 1000 == 0) {
			LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
			int percent = std::max(1, std::min(99, (int)((double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainActive.Height() - Params().Zerocoin_StartHeight()))));
			uiInterface.ShowProgress(_("Recalculating spent ZVERS..."), percent);
		}

		//Rewrite zVERS supply
		CBlock block;
		assert(ReadBlockFromDisk(block, pindex));
		...
		//Remove spends from zVERS supply
...
bool RecalculateVERSSupply(int nHeightStart)
{
	...
	uiInterface.ShowProgress(_("Recalculating VERS supply..."), 0);
	while (true) {
		if (pindex->nHeight % 1000 == 0) {
			LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
			int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainActive.Height() - nHeightStart))));
			uiInterface.ShowProgress(_("Recalculating VERS supply..."), percent);
		}
...
bool UpdateZVERSSupply(const CBlock& block, CBlockIndex* pindex)
{
	std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
	...
	for (unsigned int i = 0; i < block.vtx.size(); i++) {
		...
		if (tx.HasZerocoinSpendInputs()) {
			...
			// Check that zVERS mints are not already known
			if (tx.HasZerocoinMintOutputs()) {
				for (auto& out : tx.vout) {
					if (!out.IsZerocoinMint())
						continue;
	...
	//Track zVERS money supply in the block index
	if (!UpdateZVERSSupply(block, pindex))
		return state.DoS(100, error("%s: Failed to calculate new zVERS supply for block=%s height=%d", __func__,
...
void static UpdateTip(CBlockIndex* pindexNew)
{
	...
	*
	#ifdef ENABLE_WALLET
	// If turned on AutoZeromint will automatically convert VERS to zVERS
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
	...
	for (const CTransaction& tx : block.vtx) {
		...
		// double check that there are no double spent zVERS spends in this block
		if (tx.HasZerocoinSpendInputs()) {
			for (const CTxIn& txIn : tx.vin) {
				bool isPublicSpend = txIn.IsZerocoinPublicSpend();
				if (txIn.IsZerocoinSpend() || isPublicSpend) {
					...
					if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
						return state.DoS(100, error("%s : Double spending of zVERS serial %s in block\n Block: %s",
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
	...
	if (isPoS) {
		...
		// Inputs
		std::vector<CTxIn> versInputs;
		std::vector<CTxIn> zVERSInputs;

		for (const CTxIn& stakeIn : stakeTxIn.vin) {
			if (stakeIn.IsZerocoinSpend()) {
				zVERSInputs.push_back(stakeIn);
			}
			else {
				versInputs.push_back(stakeIn);
			}
		}
		const bool hasVERSInputs = !versInputs.empty();
		const bool hasZVERSInputs = !zVERSInputs.empty();
		...
		for (const CTransaction& tx : block.vtx) {
			for (const CTxIn& in : tx.vin) {
				if (nHeight >= Params().Zerocoin_StartHeight()) {
					...
					if (isPrivZerocoinSpend || isPublicSpend) {
						...
						if (isPublicSpend) {
							libzerocoin::ZerocoinParams* params = Params().Zerocoin_Params();
							PublicCoinSpend publicSpend(params);
							if (!ZVERSModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)) {
								return false;
							}
							spend = publicSpend;
						}
				...
				if (hasVERSInputs)
					...
					for (const CTxIn& versIn : versInputs) {
						if (versIn.prevout == in.prevout) {
		...
		if (isBlockFromFork) {
			...
			while (!chainActive.Contains(prev)) {
				...
				for (const CTransaction &t : bl.vtx) {
					for (const CTxIn &in : t.vin) {
						...
						for (const CTxIn &stakeIn : versInputs) {
			...
			// Now that this loop if completed. Check if we have zVERS inputs.
			if (hasZVERSInputs) {
				for (const CTxIn& zVersInput : zVERSInputs) {
					CoinSpend spend = TxInToZerocoinSpend(zVersInput);
					...
					if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex, 0))
						return state.DoS(100, error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
							stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zvers");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
	...
	if (nMints || nSpends)
		LogPrintf("%s : block contains %d zVERS mints and %d zVERS spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
	...
	if (strCommand == "version") {
		...
		// VERSESS: We use certain sporks during IBD, so check to see if they are
		...
		if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
			try {
				...
				if (!GetAccumulatorValue(height, den, bnAccValue)) {
					LogPrint("zvers", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
					Misbehaving(pfrom->GetId(), 50);
				}
				else {
			...
			try {
				...
				if (gen.isValid(chainActive.Height())) {
					if (!lightWorker.addWitWork(gen)) {
						LogPrint("zvers", "%s : add genwit request failed \n", __func__);
						CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
