...
CzVERSWallet::CzVERSWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zvers seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZVERSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZVERSSeed_deprecated()) {
                    LogPrintf("%s: Updated zVERS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zvers seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zvers wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzVERSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZVERSCount(nCountLastUsed);
    else if (!walletdb.ReadZVERSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzVERSWallet::Lock()
{
    seedMaster = 0;
}

void CzVERSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzVERSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZVERS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzVERSWallet::LoadMintPoolFromDB()
{
...
void CzVERSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzVERSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzVERSWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zversTracker->HasPubcoinHash(pMint.first)) {
...
bool CzVERSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZVERS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zversTracker which also adds to database
    pwalletMain->zversTracker->Add(dMint, true);
...
void CzVERSWallet::SeedToZVERS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzVERSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzVERSWallet::GenerateDeterministicZVERS(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzVERSWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzVERSWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzVERSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
