...
CzVERSTracker::CzVERSTracker(std::string strWalletFile)
{
...
CzVERSTracker::~CzVERSTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzVERSTracker::Init()
{
...
bool CzVERSTracker::Archive(CMintMeta& meta)
{
...
bool CzVERSTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzVERSTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzVERSTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzVERSTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzVERSTracker::GetSpendCache(const uint256& hashStake)
{
...
std::vector<uint256> CzVERSTracker::GetSerialHashes()
{
...
CAmount CzVERSTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzVERSTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzVERSTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzVERSTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzVERSTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzVERSTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzVERSTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzVERSTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzVERSTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzVERSTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzVERSTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzVERSWallet* zVERSWallet)
{
    bool iszVERSWalletInitialized = (NULL != zVERSWallet);
    CMintMeta meta;
    ...
    if (! iszVERSWalletInitialized)
        zVERSWallet = new CzVERSWallet(strWalletFile);
    meta.isSeedCorrect = zVERSWallet->CheckSeed(dMint);
    if (! iszVERSWalletInitialized)
        delete zVERSWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzVERSTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzVERSTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzVERSTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzVERSTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzVERSTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzVERSTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzVERSWallet* zVERSWallet = new CzVERSWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zVERSWallet);
        }
        delete zVERSWallet;
        LogPrint("zero", "%s: added %d dzvers from DB\n", __func__, listDeterministicDB.size());
    }
